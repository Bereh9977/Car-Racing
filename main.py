...
class GameSys:
    def __init__(self):
        ...
        self.pause_font = pygame.font.Font("D:/course2/SEM2/CICD/Fonts/AveriaSansLibre-Bold.ttf", 36)
        self.pause_text = self.pause_font.render('Paused', True, (255, 255, 255))
        self.pause_rect = self.pause_text.get_rect(center=(self.aspect_ratio[0]//2, self.aspect_ratio[1]//2))
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        if not self.handle_pause():  # Викликаємо метод паузи
                            return  # Якщо handle_pause повернув False, виходимо
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.menu.start_rect.collidepoint(event.pos):
                        overlay_music_in_loop("D:/course2/SEM2/CICD/soundeffects/button_sound.mp3")
                        self.in_menu = False
                        self.choosing_mode = True
                        self.show_modes()
                    elif self.menu.options_rect.collidepoint(event.pos):
                        overlay_music_in_loop("D:/course2/SEM2/CICD/soundeffects/button_sound.mp3")
                        self.menu.show_options(self.screen, self.running, self.in_menu, self.background)  
        ...
        while paused:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_ESCAPE:
                        return True  # Повертаємо True, щоб продовжити гру
...
class Menu:
    ...
    def countdown(self, countdown_images, screen, roads, car, car1, car2, bot, aspect_ratio, mode_choice):
        overlay_music_in_loop("D:/course2/SEM2/CICD/soundeffects/321go.mp3")
    ...
    def show_options(self, screen, running, in_menu, background):
        options_active = True  
...
class Cars:
    ...
    def obstakles_feaches(self, obs, map_choice):
        if obs.check_collision_obstackles(self.rect, map_choice):  # Якщо машина торкається об'єкта
            if map_choice == "winter":
                self.frozen = True
                overlay_music_in_loop("D:/course2/SEM2/CICD/soundeffects/ice_sound.mp3")
                ...
                overlay_music_in_loop("D:/course2/SEM2/CICD/soundeffects/spinning.mp3")
    ...
    def check_collision(self, car_rect):
        ...
        for i, rect in enumerate(self.coin_rects):
            if rect.colliderect(car_rect):
                ...
                overlay_music_in_loop("D:/course2/SEM2/CICD/soundeffects/coin_collect.mp3")
                self.add_score(100)
                return True
        return False
    ...
    def check_click(self, pos):
        ...
        if self.buy_image_rect.collidepoint(pos):
            if self.score.purchase_item("beach", "map"):
                overlay_music_in_loop("D:/course2/SEM2/CICD/soundeffects/purchase.mp3")
...
class Finish:
    ...
    def display_circle_number(self, screen, aspect_ratio):
        ...
        :param aspect_ratio: Співвідношення сторін екрану.
        """
        font = pygame.font.Font("D:/course2/SEM2/CICD/Fonts/AveriaSansLibre-Bold.ttf", 72)
