...
class GameSys:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    if self.in_menu:
                        self.running = False  # Вихід з гри, якщо в головному меню
                    else:
                        self.paused = not self.paused  # Заморозка гри
    ...
    def show_menu(self):
        while self.in_menu:
            ...
            self.menu.draw_music_toggle(self.screen, self.music_on)  # Малюємо кнопку вимкнення музики
            pygame.display.update()
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    self.running = False  # Вихід при натисканні ESC
...
class Menu:
    def __init__(self, x, y):
        ...
        self.music_toggle_rect = pygame.Rect(x, y + 200, 200, 50)  # Кнопка для вимкнення музики
